...
class MS_SSIM_L1_LOSS(nn.Module):
    ...
    def forward(self, x, y):
        b, h, w = x.shape
        x = x.unsqueeze(1).expand(-1, 3, -1, -1)  # 채널 차원을 3으로 확장
        y = y.unsqueeze(1).expand(-1, 3, -1, -1)  # 채널 차원을 3으로 확장
        ...
        loss_mix = self.alpha * loss_ms_ssim.squeeze(1) + (1 - self.alpha) * gaussian_l1 / self.DR
...
class Trainer(object):
    def __init__(self, 
        ...
        if warp_scheduler is None:
            self.warp_scheduler = optim.lr_scheduler.LambdaLR(self.optimizer, lr_lambda=lambda ephoch: 1)
        else:
    ...
    def train_step(self, data):
        ...
        loss_function = MS_SSIM_L1_LOSS(gaussian_sigmas=[0.5, 1.0, 2.0, 4.0, 8.0], data_range=1.0, alpha=0.5)
        loss = loss_function(pred_rgb, rgb)
